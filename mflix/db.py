...
def get_movies(filters, page, movies_per_page):
    ...
    movies = cursor.skip(int(movies_per_page) * int(page)).limit(movies_per_page)
...
def add_user(name, email, hashedpw):
    ...
    try:
        ...
        if not insert_result.acknowledged:
            return {"error": "The server has not acknowledged this request"}
        return {"success": True}
